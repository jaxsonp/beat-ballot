...
def create_playlist(spotify, session_token, playlist_name: str) -> Response:
    ...
    print(f"Creating playlist \"{playlist_name}\" ", end="")
    ...
    result = spotify.user_playlist_create(os.environ["USER_ID"], playlist_name, public=True)
    playlist_uri = result["id"]


    cursor.execute(f"INSERT INTO Playlists (PlaylistURI) VALUES (\'{playlist_uri}\');")

    playlist_id = cursor.execute(f"SELECT PlaylistID FROM Playlists WHERE PlaylistURI=\"{playlist_uri}\";").fetchone()[0]
